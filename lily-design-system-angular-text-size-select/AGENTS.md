...
A reusable Angular 20 headless text-size select. The control is an
...
persistence. Ships no CSS; consumer styles the `text-size-select`
...
- `TextSizeSelectIcon` (optional marker directive,
...
`detectFromSystem` counterpart to theme-select's — no OS "preferred
...
`@for` is used (not `*ngFor`). Ids come from `nextTextSizeSelectId()`,
