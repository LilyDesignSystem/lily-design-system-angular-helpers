 ...
 * `themeName` in theme-select and `localeName` in locale-select.
 */
 ...
 *   <ng-template lilyTextSizeSelectIcon let-args>{{ args.labelFor(args.value) }}</ng-template>
 * </lily-text-size-select>
 * ```
 *
 ...
 */
@Directive({
  selector: "ng-template[lilyTextSizeSelectIcon]",
  standalone: true,
})
export class TextSizeSelectIcon {
  static ngTemplateContextGuard(
    _dir: TextSizeSelectIcon,
    _ctx: unknown,
    ...
    >
      ...
      >
        @if (iconTemplate(); as tpl) {
          ...
          <span class="text-size-select-icon" aria-hidden="true">{{ glyph }}</span>
        }
      </button>
...
export class TextSizeSelect {
  ...
  private readonly baseId = nextTextSizeSelectId();
