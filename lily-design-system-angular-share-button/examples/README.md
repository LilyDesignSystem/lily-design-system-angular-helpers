# Examples — ShareButton

Self-contained Angular 20 examples for
`lily-design-system-angular-share-button`. Each file is a runnable
...
- No CSS dependency — the control is headless. Consumers style the
  `share-button` (root), `share-button-trigger`, `share-button-icon`,
  `share-button-list`, `share-button-list-item`, `share-button-target`,
  `share-button-copy`, and `share-button-status` hooks.
- **The list needs positioning CSS and this package ships none.** The
  `<ul class="share-button-list">` sits in normal document flow, so it
...
2. Import `ShareButton` from this directory (or via the `index.ts`
   barrel).
