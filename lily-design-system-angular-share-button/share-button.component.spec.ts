...
/** Create + render a ShareButton with the supplied inputs. */
function mount(inputs: Record<string, unknown> = {}): ComponentFixture<ShareButton> {
...
function trigger(fixture: ComponentFixture<unknown>): HTMLButtonElement {
  return q<HTMLButtonElement>(fixture, ".share-button-trigger");
}

function list(fixture: ComponentFixture<unknown>): HTMLUListElement {
  return q<HTMLUListElement>(fixture, ".share-button-list");
}

function links(fixture: ComponentFixture<unknown>): HTMLAnchorElement[] {
  return Array.from(
    fixture.nativeElement.querySelectorAll(".share-button-target"),
  ) as HTMLAnchorElement[];
}

function items(fixture: ComponentFixture<unknown>): HTMLElement[] {
  return Array.from(
    fixture.nativeElement.querySelectorAll(".share-button-target, .share-button-copy"),
  ) as HTMLElement[];
}

function status(fixture: ComponentFixture<unknown>): HTMLElement {
  return q<HTMLElement>(fixture, ".share-button-status");
}
...
describe("ShareButton — markup contract (§4.2, §7.1–§7.6)", () => {
  ...
  test("§4.2 the trigger's class hook is share-button-trigger, not -button", () => {
    const fixture = mount();
    expect(trigger(fixture)).toBeTruthy();
    expect(q(fixture, ".share-button-button")).toBeNull();
  });

  test("§4.2 the root carries the base class plus the consumer's class", () => {
    const fixture = mount({ className: "extra" });
    const root = q<HTMLElement>(fixture, ".share-button");
  ...
  test("§7.1 the button renders ↪, hidden from assistive tech", () => {
    const fixture = mount();
    const icon = q<HTMLElement>(fixture, ".share-button-icon");
  ...
  test("§4.3 nextShareButtonId mints unique, stable ids", () => {
  ...
  test("§7.3 destinations sit in .share-button-list-item children", async () => {
    const fixture = mount();
    await clickSettled(fixture, trigger(fixture));
    const li = fixture.nativeElement.querySelectorAll(".share-button-list-item");
...
describe("ShareButton — copy to clipboard (§7.7–§7.10)", () => {
  test("§7.7 copying writes the URL and emits copy", async () => {
    ...
    await clickSettled(fixture, trigger(fixture));
    await clickSettled(fixture, q<HTMLElement>(fixture, ".share-button-copy"));
...
describe("ShareButton — keyboard and dismissal (§7.15–§7.19)", () => {
  ...
  ): Promise<ComponentFixture<ShareButton>> {
...
describe("ShareButton — url resolution (§7.20–§7.21)", () => {
...
@Component({
  ...
  template: `
    <lily-share-button label="Share" [targets]="targets" [url]="url">
...
describe("ShareButton — custom glyph template (§7.22)", () => {
  test("§7.22 a projected ng-template replaces the glyph and receives ChildArgs", async () => {
    ...
    expect(custom.closest("button")?.className).toContain("share-button-trigger");
    expect(q(fixture, ".share-button-icon")).toBeNull();
