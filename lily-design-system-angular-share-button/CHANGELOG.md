...
- Initial release. An Angular 20 port of the canonical Svelte
  `share-button` helper: a headless share control whose single-glyph
...
- A projected `<ng-template>` replaces the glyph and receives
  `ChildArgs` (`{ open, url }`); the optional `ShareButtonIcon` marker
...
- Unlike the `*-select` helpers, this owns an *action*, not a
