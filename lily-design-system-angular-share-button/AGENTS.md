# AGENTS — ShareButton (Angular helper)
...
| `share-button.component.ts`         | Implementation. Standalone, signal-based, OnPush. |
| `share-button.component.spec.ts`    | Vitest spec, mapped to the §7 clauses (47 cases). |
...
- `ShareButton` (component class, selector `lily-share-button`).
- `ShareButtonIcon` (optional marker directive,
  `ng-template[lilyShareButtonIcon]`, for typed `let-` variables).
- `RIGHTWARDS_ARROW_WITH_HOOK` (the default glyph, `"↪"` U+21AA).
- `canShareNatively`, `canCopy` (pure, SSR-safe capability probes).
- `nextShareButtonId` (per-instance id generator).
...
and copy-link-address. The trigger class is `share-button-trigger`, not
`-button`, because `.share-button-button` reads badly — the one
deliberate bend in the `{helper}-button` convention.

`@for` is used (not `*ngFor`), tracked by `target.id`. Ids come from
`nextShareButtonId()`, an incrementing module counter — stable, unique
