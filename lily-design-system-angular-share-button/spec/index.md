...
Single source of truth for the `lily-design-system-angular-share-button`
...
[spec](../../../lily-design-system-svelte-helpers/lily-design-system-svelte-share-button/spec/index.md);
per `AGENTS/helpers.md`, Svelte wins where the catalogs disagree. This
file states the same contract in Angular 20 idiom.

Sibling files:

- `share-button.component.ts` — the implementation
- `share-button.component.spec.ts` — vitest spec exercising every clause in §7
...
optional `ShareButtonIcon` marker directive
(`ng-template[lilyShareButtonIcon]`) types the `let-` variables. The
...
`share-button.component.spec.ts` asserts every clause below. Each clause
...
- *destinations sit in .share-button-list-item children* — one `<li>` per
  destination.
...
- *a projected ng-template replaces the glyph and receives ChildArgs* —
  the custom node sits inside `.share-button-trigger`, the default
  `.share-button-icon` is gone, and the context carries `open` and `url`.
...
- *§4.2 the trigger's class hook is share-button-trigger, not -button*.
- *§4.2 the root carries the base class plus the consumer's class*.
- *§4.3 nextShareButtonId mints unique, stable ids*.
...
- Package: lily-design-system-angular-share-button
- Version: 0.1.0
- License: MIT
