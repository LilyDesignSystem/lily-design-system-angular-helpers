 ...
 *
 * ```html
 * <lily-share-button label="Share">
 *   <ng-template lilyShareButtonIcon let-args>{{ args.open ? "×" : "↪" }}</ng-template>
 * </lily-share-button>
 * ```
 *
 ...
 */
@Directive({
  selector: "ng-template[lilyShareButtonIcon]",
  standalone: true,
})
export class ShareButtonIcon {
  static ngTemplateContextGuard(
    _dir: ShareButtonIcon,
    _ctx: unknown,
...
/**
 * ShareButton — a headless share control.
 *
    ...
    >
      ...
      >
        @if (iconTemplate(); as tpl) {
          ...
          <span class="share-button-icon" aria-hidden="true">{{ glyph }}</span>
        }
      </button>
      ...
      >
        ...
        @if (copyLabel()) {
          <li class="share-button-list-item">
            <button type="button" class="share-button-copy" (click)="copyUrl()">
              {{ copyLabel() }}
            </button>
          </li>
        }
...
export class ShareButton {
  ...
  private readonly baseId = nextShareButtonId();
