# MotionPicker (Angular helper)

A reusable Angular 20 headless **motion (reduced-motion) picker** — an
...
| `value`        | `string` (model)         | no       | Selected slug. Two-way bindable via `[(value)]`.               |
...
| `motionChange` | `output<string>`         | —        | Emits after a new motion preference is applied.                |
...
All DOM writes guard on `typeof document`, so the component is
SSR-safe.
