# MotionPicker — Specification (Angular helper)
...
aria-controls>` whose only content is the `aria-hidden` icon — a
bundled two-bar-pause SVG (`viewBox="0 0 16 16"`, `<path d="M5 3v10M11
3v10"/>`), not a Unicode character (reversed 2026-09-16), replaceable
via `children` — and a `<ul class="motion-picker-list"
...
2026-09-16 from a Unicode glyph; the removed `PAUSE_SIGN` constant was
not renamed). It is additive: existing catalogs, counts, and contracts
for the other four helpers are unchanged by its introduction.
