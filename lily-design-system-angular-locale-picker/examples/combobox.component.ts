/*
    ...
    Note on `className="locale-picker-hidden"`: the helper still owns
  ...
  template: `
    ...
    <datalist id="locale-options">
      @for (l of locales(); track l) {
        <option [value]="labelFor(l)" [attr.lang]="tagFor(l)">{{ l }}</option>
      }
    </datalist>

    <p>
      Selected locale: <code>{{ locale() }}</code> ({{ labelFor(locale()) }})
    </p>
  `,
})
