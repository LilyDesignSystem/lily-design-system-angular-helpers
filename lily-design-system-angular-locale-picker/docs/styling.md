...
| `.locale-chooser-button`               | The trigger `<button>`. Icon-only by default. |
| `.locale-chooser-icon`                 | The `<span aria-hidden="true">` holding the default globe glyph. Absent when a custom `<ng-template>` is projected. |
| `.locale-chooser-list`                 | The `<ul role="listbox">`. Carries `hidden` while closed. |
| `.locale-chooser-option`               | Each `<li role="option">`. Carries its own `lang`. |
| `.locale-chooser-status`               | The consumer-rendered status line naming the active locale. Not emitted by the component — you render it, per the default pattern in [accessibility.md](./accessibility.md#the-status-region-is-part-of-the-pattern). |

## State selectors

| Selector                                      | Meaning                                                     |
| --------------------------------------------- | ----------------------------------------------------------- |
| `.locale-chooser-option[data-active]`          | The option the **keyboard** is currently pointing at.        |
| `.locale-chooser-option[aria-selected="true"]` | The locale currently **in effect**.                          |
| `.locale-chooser-button[aria-expanded="true"]` | The listbox is open.                                         |
| `.locale-chooser-list[hidden]`                 | The listbox is closed (the browser hides it; don't override).|
...
Do **not** write `.locale-chooser-list { display: block }` or similar
...
it: `.locale-chooser-list:not([hidden]) { display: grid }`.
...
- **Don't set a direction on the options.** The Unicode Bidirectional
  ...
  Forcing `direction: ltr` on `.locale-chooser-option` will visually
...
The `.locale-chooser-list:focus-visible` rule is not optional
...
`:focus-visible` style on `.locale-chooser-option` instead is the
...
beside theme-chooser's `◑` in a page header, and the two should read as
one set.
...
stylesheet the sibling theme-chooser helper has loaded. Because both
...
- Don't force `.locale-chooser-list` visible with an unconditional
...
- Don't put `:focus-visible` styles on `.locale-chooser-option`. The
...
in another component must publish their locale-chooser-targeting styles
...
@Component({
    ...
    encapsulation: ViewEncapsulation.None,  // styles reach .locale-chooser
    template: `
...
Note also that `<lily-locale-chooser>` itself is an element in the DOM,
sitting between your wrapper and `.locale-chooser`. If your positioning
context needs to be that element rather than the root div, style
`lily-locale-chooser { position: relative }` instead — but prefer
`.locale-chooser`, which is the documented hook.
