...
| `.locale-picker-icon`            | The `<span aria-hidden="true">` holding the default globe glyph. Absent when a custom `<ng-template>` is projected.                                                                                                   |
...
| `.locale-picker-status`          | The consumer-rendered status line naming the active locale. Not emitted by the component — you render it, per the default pattern in [accessibility.md](./accessibility.md#the-status-region-is-part-of-the-pattern). |

## State selectors

| Selector                                       | Meaning                                                       |
| ---------------------------------------------- | ------------------------------------------------------------- |
| `.locale-picker-option[data-active]`          | The option the **keyboard** is currently pointing at.         |
| `.locale-picker-option[aria-selected="true"]` | The locale currently **in effect**.                           |
| `.locale-picker-button[aria-expanded="true"]` | The listbox is open.                                          |
| `.locale-picker-list[hidden]`                 | The listbox is closed (the browser hides it; don't override). |
...
tell what they are about to change _from_.
...
Do **not** write `.locale-picker-list { display: block }` or similar
...
it: `.locale-picker-list:not([hidden]) { display: grid }`.
...
- **Don't set a direction on the options.** The Unicode Bidirectional
  ...
  Forcing `direction: ltr` on `.locale-picker-option` will visually
...
- **Don't right-align "because it's RTL".** `text-align: start`
  aligns the whole list to the reading edge of the _page_, which is
...
The `.locale-picker-list:focus-visible` rule is not optional
...
`:focus-visible` style on `.locale-picker-option` instead is the
...
stylesheet the sibling theme-picker helper has loaded. Because both
...
- Don't force `.locale-picker-list` visible with an unconditional
...
- Don't put `:focus-visible` styles on `.locale-picker-option`. The
...
in another component must publish their locale-picker-targeting styles
...
@Component({
  ...
  encapsulation: ViewEncapsulation.None, // styles reach .locale-picker
  template: `
    <lily-locale-picker
      label="Language"
      [locales]="locales"
...
Note also that `<lily-locale-picker>` itself is an element in the DOM,
sitting between your wrapper and `.locale-picker`. If your positioning
context needs to be that element rather than the root div, style
`lily-locale-picker { position: relative }` instead — but prefer
`.locale-picker`, which is the documented hook.
