...
| APG Radio Group pattern | Single-selection radio group; native radios deliver the tab/arrow contract. |
...
the `lang` attribute makes the reader switch to a French voice
for the duration of that span.
...
For 9+ locales, a sibling `<select>` bound to the same
`[(value)]` signal is the right pattern. Native `<select>` is
fully accessible:
...
as LTR. So Tab still moves through the radios in the order they
appear in your `locales` array, and Arrow Right (in RTL) moves to
the previous option, not the next. This is the browser's job,
not yours.

## Angular-specific notes

- The picker binds `aria-label` via `[attr.aria-label]="label() || null"`.
  This removes the attribute when empty rather than emitting
  `aria-label=""`.
- `[attr.lang]` (not `lang`) is used on each `<label>` so empty
