...
- **Consumers** own the visual style of the control via the
  `theme-picker` class hooks — including positioning the list, which
...
- **Ids** come from an incrementing module counter
  (`nextThemePickerId()`), so they are stable, unique per instance,
...
- **The list never closes.** An unconditional `display` rule on
  `.theme-picker-list` overrides the `hidden` attribute. Scope it
  with `:not([hidden])`.
