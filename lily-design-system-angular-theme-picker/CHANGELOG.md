...
relative to `lily-design-system-angular-theme-picker`'s last published version (`0.3.0`);
...
- **Default icon changed from a Unicode glyph to a bundled SVG.** The
  button's `theme-picker-icon` now renders an inline
  `<svg viewBox="0 0 16 16" aria-hidden="true">` (contrast/half-circle design,
  ...
  no missing-glyph "tofu" risk, no per-glyph optical-scale correction to maintain (the
  45 root `themes/*.css` files' `--lily-picker-icon-scale` rule is
  dropped for this icon; an SVG's ink fills its own `viewBox` by
  construction). The exported glyph constant **`CIRCLE_WITH_RIGHT_HALF_BLACK`**
  (◑ U+25D1) is **removed, not renamed** — there is
