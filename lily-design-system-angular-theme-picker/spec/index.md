...
Single source of truth for the `lily-design-system-angular-theme-chooser`
...
- `theme-chooser.component.ts` — the implementation
- `theme-chooser.component.spec.ts` — vitest spec exercising every clause in §4–§7
- `index.ts` — re-export barrel
- `index.md` — user-facing readme

The companion headless catalog entry
(`lily-design-system-angular-headless/components/ThemeChooser.ts`) is a
...
Give an Angular 20 application a drop-in, headless theme chooser that:
...
- **Per-instance ids from a module counter.** `nextThemeChooserId()`
...
- **One `<link>` per select name.** Switching themes mutates `href`
  ...
  data-lily-theme-chooser="{name}">`. Multiple choosers can coexist by
...
- **Custom glyph via a projected `<ng-template>`.** The Svelte
  ...
  `ThemeChooserIcon` marker directive (`ng-template[lilyThemeChooserIcon]`)
...
| `detectFromSystem` | `input<boolean>()`               | no       | `false`                          | Resolve `prefers-color-scheme` to a supported theme on first visit. Mirrors `detectFromNavigator` on locale-chooser. |
| `name`          | `input<string>()`                   | no       | `"theme"`                        | `name` attribute on the hidden input **and** the discriminator on the managed `<link data-lily-theme-chooser="{name}">`. |
...
| `className`     | `input<string>()`                   | no       | `""`                             | Extra CSS class on the root `<div>`, appended after `theme-chooser`. |
...
`<lily-theme-chooser>` replaces the default glyph inside the button.
...
<div class="theme-chooser {className}">
  ...
  <ul class="theme-chooser-list" id="{listId}" role="listbox"
    ...
    <li class="theme-chooser-option" id="{listId-derived optionId}"
        role="option" aria-selected="true" data-active>Light</li>
    <li class="theme-chooser-option" id="…" role="option"
        aria-selected="false">Dark</li>
  </ul>
</div>
```

- **Root**: a `<div>` carrying the `theme-chooser` class hook plus the
...
- **Glyph**: `<span class="theme-chooser-icon" aria-hidden="true">`
...
- **Listbox**: `<ul class="theme-chooser-list" role="listbox">` with the
...
- **Ids**: `nextThemeChooserId()` returns `theme-chooser-{n}` from an
...
- **Custom glyph**: a projected `<ng-template>` (queried with
  ...
  `.theme-chooser-icon` span inside the button. Its context is
...
- `labelFor(slug)` returns `themeLabels[slug]` when supplied;
  ...
  re-deriving it, mirroring `localeName` on locale-chooser. The select
...
- `ThemeChooser` (the component class)
- `ThemeChooserIcon` (the optional icon-template marker directive)
- `CIRCLE_WITH_RIGHT_HALF_BLACK` (the default glyph constant)
- `nextThemeChooserId` (the per-instance id generator)
...
occupies for locale-chooser, so the two helpers resolve symmetrically:
...
`theme-chooser.component.spec.ts` must assert every numbered clause
...
**7.1 — Skeleton.** The root is a `<div class="theme-chooser">`
...
`aria-controls` matching the `id` of a `<ul class="theme-chooser-list"
...
**7.3 — Options and hidden input.** One `<li class="theme-chooser-option">`
...
data-lily-theme-chooser="{name}">` exists in `document.head` with
...
no `data-lily-theme-chooser="theme"` link is created.
...
root `<div>`'s class list after `theme-chooser`.

**7.13 — Custom glyph.** A projected `<ng-template>` replaces the
default `.theme-chooser-icon` span inside the button (the default span
...
- Package directory: `lily-design-system-angular-helpers/lily-design-system-angular-theme-chooser/`
- Spec version: 0.1.0
- Created: 2026-06-05
