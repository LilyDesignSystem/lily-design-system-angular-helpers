...
### Changed (symmetry with theme-select)
...
- **The default globe glyph gains U+FE0E VARIATION SELECTOR-15.**
  ...
  and the globe renders blue, which does not match theme-select's
...
- **Five shared topic guides**, so locale-select and theme-select now
  offer the same doc shape: `docs/props-reference.md`,
  `docs/styling.md`, `docs/custom-rendering.md`, `docs/recipes.md`,
  and `docs/troubleshooting.md`. Written for locale-select rather
  than adapted from the theme-select copies. The topic-specific
...
- **Examples move from the radio-group era's ordinal names to
  descriptive ones**, matching theme-select's convention. None of the
...
- **The control is no longer a native `<select>`.** It is now an icon
  ...
  `<div class="locale-select {className}">` containing a
  `<button class="locale-select-button">` with
  `aria-haspopup="listbox"`, `aria-expanded`, and `aria-controls`, and
  a `<ul class="locale-select-list" role="listbox">` of
  `<li class="locale-select-option" role="option">` children. Any
  consumer CSS, test, or e2e selector targeting `select.locale-select`
  or `option.locale-select-option` must be updated.
- **The `placeholder` input is removed.** With no `<select>` to pin,
  the "pinned placeholder keeps the control narrow" tradeoff from
  0.3.0 no longer exists. The `locale-select-placeholder` class hook
...
- `locale-select-button`, `locale-select-icon`, and
  `locale-select-list` class hooks. The package still ships zero CSS.
...
- `GLOBE_WITH_MERIDIANS` export — the default button glyph, U+1F310
  ...
  `aria-hidden` span.
- `LocaleSelectIcon` export — an optional marker directive
  (`ng-template[lilyLocaleSelectIcon]`) giving consumers typed `let-`
...
- `nextLocaleSelectId` export — the module-counter id generator behind
