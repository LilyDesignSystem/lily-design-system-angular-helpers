...
| **Writing direction**      | The `dir` attribute on `<html>`. Bidi text, scrollbar position, flex/grid mirror. | `LocaleSelect` (auto-detected from the locale; opt out with `[applyDir]="false"`). |
...
attribute lives on the `<lily-locale-select>` element rather than
on `document.documentElement`. The current `LocaleSelect` writes
