...
`LocaleSelect` exposes exactly one rendering escape hatch: a projected
...
default `<span class="locale-select-icon" aria-hidden="true">🌎︎</span>`
inside the button.
...
## Typed `let-` variables with `LocaleSelectIcon`

By default `let-args` is implicitly `any`. Add the exported
`LocaleSelectIcon` marker directive to get `ChildArgs` typed under
`strictTemplates`:
...
@Component({
    ...
    standalone: true,
    imports: [LocaleSelect, LocaleSelectIcon],
...
The directive's selector is `ng-template[lilyLocaleSelectIcon]` and its
...
<lily-locale-select label="Language" [locales]="locales" [(value)]="locale">
    <ng-template let-value="value">
        <span class="locale-select-code">{{ value.slice(0, 2).toUpperCase() }}</span>
    </ng-template>
</lily-locale-select>
```
...
<lily-locale-select label="Language" [locales]="locales" [(value)]="locale">
    <ng-template let-args>
        <span class="locale-select-caret" [class.is-open]="args.open" aria-hidden="true">
            {{ args.open ? "▴" : "▾" }}
        </span>
    </ng-template>
</lily-locale-select>
...
`.locale-select-button[aria-expanded="true"]` is available as a
selector, and a rotation reads better than a glyph swap:

```css
.locale-select-button[aria-expanded="true"] .locale-select-caret {
...
- **`.locale-select-icon` is not rendered when a template is
...
template. Keep `LocaleSelect` mounted so it owns the behaviour, hide
...
**The hiding rule is the part people get wrong.** `locale-select-hidden`
