...
2. **DevTools** — Chrome's element panel can toggle `dir="rtl"`
   on `<html>` directly. Firefox has the same in its inspector.
3. **Automated** — add a Playwright spec that flips the select
   and asserts `document.documentElement.dir === "rtl"` and (e.g.)
   that a visually-critical element's bounding box is on the
   right side of its parent.

## Angular-specific notes

### Reactive `dir` via `effect()`

When pre-resolving the locale server-side, write `dir` via a
server-only environment initializer (see [./ssr.md](./ssr.md)).
The select still writes `dir` on client-side mutation, but the
SSR-side write keeps the first-paint markup correct.

### `:dir()` selector in Angular component styles

`:dir(rtl)` works in Angular component styles the same as
anywhere — but Angular's `ViewEncapsulation.Emulated` prefixes
your selectors with a scoping attribute, so `:dir(rtl) :host`
becomes nonsense. Use `:host-context(:dir(rtl))` instead:

```css
:host-context(:dir(rtl)) .my-icon {
    transform: scaleX(-1);
}
```

`:host-context()` lets a component query its ancestor chain
for selectors without breaking encapsulation.

### `Dir` directive from `@angular/cdk/bidi`

Angular CDK's `BidiModule` provides a `Dir` directive and a
`Directionality` service that emit RTL/LTR changes. These are
useful in your own components when you need to react to
`dir="rtl"` programmatically — but the helper itself writes
imperatively to the DOM, so it doesn't depend on the CDK.
