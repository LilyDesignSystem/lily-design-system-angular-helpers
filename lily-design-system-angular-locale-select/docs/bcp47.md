...
well-formed tags). It does **not** implement RFC 4647 matching
(it does a simple two-step exact-then-prefix match in
...
You only need a script subtag when the language is written in
more than one script. Common ones:
...
right-to-left and trigger `dir="rtl"` in this helper regardless
of the base language. See [./rtl.md](./rtl.md).
...
format (`2026-06-05` is "June 5" vs "5 June"), and currency
symbol position.
...
2021) which translates BCP 47 tags into human names in any
locale:
...
Angular's own `LOCALE_ID` token controls the default locale for
`DatePipe`, `CurrencyPipe`, etc. The helpers do **not** mutate
`LOCALE_ID` — it's set at app bootstrap and stays static. The
...
bootstrapApplication(App, {
    providers: [
        { provide: LOCALE_ID, useFactory: () => inject(LocaleStore).current() },
    ],
});
```

This is a consumer choice; the helper stays neutral.

### `$localize` template literals

`@angular/localize`'s `$localize` template literals extract at
...
**Should I include the script subtag?** Only when the language
is written in multiple scripts and the choice matters. `zh-Hans`,
`zh-Hant`, `sr-Cyrl`, `sr-Latn` — yes. `en-Latn` — no, English
is implicitly Latin and `Suppress-Script: Latn` in the registry
says so.

**How do I match `en-GB` to `en-AU`?** That's RFC 4647
territory. Either implement Lookup yourself (this helper does a
simpler two-step match) or pre-flatten your locale list so
language-only fallbacks exist.
