...
**Cause.** The package ships zero CSS, and the `<ul class="locale-select-list">`
is an ordinary block element in normal document flow until you take it out.

**Fix.** `position: relative` on `.locale-select`, `position: absolute` on
`.locale-select-list`. Full recipe in [styling.md](./styling.md).

## "The list never closes"

**Cause.** An unconditional `display` rule —
`.locale-select-list { display: block }` — overrides the `hidden` attribute
the component toggles.

**Fix.** Scope it: `.locale-select-list:not([hidden]) { display: block }`.
...
**Cause.** `LocaleSelect` is not a translation library. `applyLocale` writes
...
**Probably not the ids.** `nextLocaleSelectId()` is a module-scoped counter,
...
presentation so the glyph stays monochrome next to theme-select's `◑`. The
...
selector is gone. Then set a font stack on `.locale-select-icon` that includes
...
U+1F310 at all. Give `.locale-select-button` a `min-width` and `min-height` so
...
`LocaleSelect` never calls `matchMedia` — it has no media-query behaviour of
...
Add the exported `LocaleSelectIcon` marker directive to the `<ng-template>`
...
**Cause.** No focus ring on `.locale-select-list`. The `<ul>` — not the
...
**Fix.** Style `.locale-select-list:focus-visible` alongside
`.locale-select-button:focus-visible`. See
