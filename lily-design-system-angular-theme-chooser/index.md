# ThemeChooser (Angular helper)

A reusable, headless Angular 20 theme chooser that **loads themes
...
- **Consumers** own the visual style of the control via the
  `theme-chooser` class hooks — including positioning the list, which
...
```ts
import { ThemeChooser } from "./lily-design-system-angular-theme-chooser";
```

The barrel also re-exports the optional `ThemeChooserIcon` marker
directive, the `CIRCLE_WITH_RIGHT_HALF_BLACK` glyph constant, the
`nextThemeChooserId` id generator, the pure helpers
...
2. Import the standalone `ThemeChooser` component and render it.
...
which theme is in effect. The `theme-chooser-status` element restores
...
Step 3 is the mirror of locale-chooser's `detectFromNavigator`, in the
...
the mirror of locale-chooser's `localeName` — import it rather than
...
- **Ids** come from an incrementing module counter
  (`nextThemeChooserId()`), so they are stable, unique per instance,
...
`ThemeChooserIcon` marker directive
(`<ng-template lilyThemeChooserIcon let-args>`) gives typed `let-`
...
`<link>` element (`data-lily-theme-chooser="{name}"`), so two selects
sharing a `name` will fight over the same stylesheet link.

Example: [`examples/multiple-choosers.component.ts`](./examples/multiple-choosers.component.ts).
...
- **The list never closes.** An unconditional `display` rule on
  `.theme-chooser-list` overrides the `hidden` attribute. Scope it
  with `:not([hidden])`.
...
| `CLAUDE.md`                         | `@AGENTS.md`.                                    |
| `theme-chooser.component.ts`         | The component implementation.                    |
| `theme-chooser.component.spec.ts`    | vitest suite covering every spec §7 item.        |
