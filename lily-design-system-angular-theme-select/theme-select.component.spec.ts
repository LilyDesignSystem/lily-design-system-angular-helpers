...
/** Create + render a ThemeSelect with the supplied inputs. */
function mount(inputs: Record<string, unknown> = {}): ComponentFixture<ThemeSelect> {
...
): Promise<ComponentFixture<ThemeSelect>> {
...
function button(fixture: ComponentFixture<unknown>): HTMLButtonElement {
  return q<HTMLButtonElement>(fixture, ".theme-select-button");
}

function list(fixture: ComponentFixture<unknown>): HTMLUListElement {
  return q<HTMLUListElement>(fixture, ".theme-select-list");
}
...
describe("ThemeSelect — pure helpers (§7.19)", () => {
...
describe("ThemeSelect — markup contract (§4.2, §7.1–§7.5)", () => {
  ...
  test("§7.1 the root is a div carrying the class hook", () => {
    ...
    const root = q<HTMLElement>(fixture, ".theme-select");
  ...
  test("§7.1 the button renders the half-circle glyph, hidden from assistive tech", () => {
    const fixture = mount();
    const icon = q<HTMLElement>(fixture, ".theme-select-icon");
...
describe("ThemeSelect — keyboard contract (APG listbox, §7.14–§7.18)", () => {
  async function openWith(key: string): Promise<ComponentFixture<ThemeSelect>> {
    const fixture = await mountSettled();
...
describe("ThemeSelect — custom icon template (§7.12–§7.13)", () => {
  test("§7.12 className is appended to the root div", () => {
    const fixture = mount({ className: "extra" });
    expect(q<HTMLElement>(fixture, ".theme-select").classList.contains("extra")).toBe(
      true,
    );
  });
...
describe("ThemeSelect — themeName (§7.19)", () => {
...
describe("ThemeSelect — matchSystemTheme (§7.20)", () => {
...
describe("ThemeSelect — detectFromSystem (§7.20)", () => {
  afterEach(() => clearMatchMedia());
