...
Element ids come from `nextThemeSelectId()`, an incrementing module
...
- Set an explicit font stack on `.theme-select-icon` that you know
...
`.theme-select-button`, and the `.theme-select-list` while open.
...
`.theme-select-list:focus-visible` and
`.theme-select-option[data-active]` together. See
...
- After closing, the control announces nothing about the new theme.
  The `theme-select-status` live region described above is what
