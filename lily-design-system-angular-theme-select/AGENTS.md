...
consumer styles the `theme-select` class hooks and supplies the
listbox positioning.
...
- `ThemeSelectIcon` (optional marker directive,
  `ng-template[lilyThemeSelectIcon]`, for typed `let-` variables).
...
`themes[0]` — the same shape as locale-select's
...
`@for` is used (not `*ngFor`). Ids come from `nextThemeSelectId()`, an
