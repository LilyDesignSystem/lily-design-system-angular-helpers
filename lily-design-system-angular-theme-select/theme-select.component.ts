 ...
 * Mirrors `matchNavigatorLanguage` in locale-select. Returns "" when the
 ...
 */
@Directive({
  selector: "ng-template[lilyThemeSelectIcon]",
  standalone: true,
})
export class ThemeSelectIcon {
  static ngTemplateContextGuard(
    _dir: ThemeSelectIcon,
    _ctx: unknown,
    ...
    >
      ...
      >
        @if (iconTemplate(); as tpl) {
          ...
          <span class="theme-select-icon" aria-hidden="true">{{ glyph }}</span>
        }
      </button>
...
export class ThemeSelect {
  ...
  private readonly baseId = nextThemeSelectId();
